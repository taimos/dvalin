...
class CryptoServiceTest {
    ...
    void setUp() throws Exception {
        ...
        keyField.set(this.service, CryptoServiceTest.KMS_KEY_ID);
    }

    @Test
    void shouldEncryptStringWithContext() {
        ByteBuffer byteBuffer = ByteBuffer.wrap("foobar".getBytes(StandardCharsets.UTF_8));
        String stringToEncrypt = "toEncrypt";

        EncryptResult result = new EncryptResult().withKeyId(CryptoServiceTest.KMS_KEY_ID).withCiphertextBlob(byteBuffer);
        ...
        Assertions.assertEquals(CryptoServiceTest.KMS_KEY_ID, request.getKeyId());
        Assertions.assertArrayEquals(stringToEncrypt.getBytes(StandardCharsets.UTF_8), request.getPlaintext().array());
        Assertions.assertEquals(1, request.getEncryptionContext().size());
        Assertions.assertEquals("value", request.getEncryptionContext().get("key"));
        Assertions.assertEquals(byteBuffer, buffer);
    }
}
